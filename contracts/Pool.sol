...
contract Pool is Ownable, ReentrancyGuard, Initializable, IPool {
    ...
    function mint(
        ...
        if (_tcr > 0) {
            uint256 _collateral_value =
                (_collateral_amount * _price_collateral) /
                    PRICE_PRECISION;
        ...
        if (_required_share_amount > 0) {
            ...
            _transferShareToReserve(msg.sender, _required_share_amount);
        }
        if (_collateral_amount > 0) {
            _transferCollateralToReserve(msg.sender, _collateral_amount);
        }

        IDollar(dollar).poolMint(address(this), _actual_dollar_amount);
        ITreasury(treasury).updateCollateralMintProfit(_collateral_amount);

        userMint[msg.sender].dollarAmount = userMint[msg.sender].dollarAmount + _actual_dollar_amount;
        last_mint[msg.sender] = block.number;
    }

    function collectMint() external {
        require(
            (last_mint[msg.sender] + mint_delay) <= block.number,
            "<mint_delay"
        );
        uint256 userAmount = userMint[msg.sender].dollarAmount;
        require(userAmount <= IERC20(dollar).balanceOf(address(this)), "dollar mint <= total dollar mint");

        IERC20(dollar).safeTransfer(msg.sender, userAmount);
        userMint[msg.sender].dollarAmount = 0;
    }
    ...
    function redeem(
        ...
        if (_ecr > 0) {
            uint256 _collateral_output_value =
                ((_dollar_amount_post_fee * _ecr) / PRICE_PRECISION);
        ...
        ITreasury(treasury).updateCollateralRedeemProfit(_collateral_output_amount);
    }
    ...
    function collectRedemptionWithoutShare() external {
        require(
            (last_redeemed[msg.sender] + redemption_delay) <= block.number,
            "<redemption_delay"
        );
    ...
    function _transferShareToReserve(address _sender, uint256 _amount) internal {
        address _reserve = collateralReserve();
        require(_reserve != address(0), "Invalid reserve address");
        ERC20(share).safeTransferFrom(_sender, _reserve, _amount);
    }
    ...
    function setRedeemUpdateOracle(bool _redeem_update_oracle) external onlyOwner {
        redeem_update_oracle = _redeem_update_oracle;
    }
